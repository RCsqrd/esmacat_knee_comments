...
void sensojoint_control::sensojoint_control_treshold_parameters() {
    // Controller LIMITS:
    torque_terms.max_velocity_threshold = 1.5;       // [rad*sec]max value allowed as motor velocity
    torque_terms.max_load_stop_threshold = 42500;   // [mNm]max value allowed as load cell reading  (12500)

    impedance_terms.T_stops_max_torque = 30000;     //[mNm]  (12500)
    impedance_terms.impedance_max_error_rad = 0.5;
    impedance_terms.max_torque_control_input = 42500;   // [mNm]  (13000)
...
//   PLOGW<<"info TorqueSetpoint"<<torque_setpoint<<endl;
 ...
 //  PLOGW<<"der_f"<<derating_factor<<endl;
...
{
    esmacat_err error = NO_ERR;
    double ref_position = esmacat_sensojoint::get_position_actual_value_rad()- ((345*M_PI)/180);
...
esmacat_err sensojoint_control::impedance_control(float setpoint_radians, float inverse_dynamics_torque_mNm, float elapsed_time_ms)
{
    esmacat_err error = NO_ERR;
 ...
 //       {PLOGW<<"vel treshold"<<max_velocity_threshold<<endl;}
    ...
    if (loadcell_reading_status == NO_ERR)
    {
        if (std::abs(loadcell_reading_mNm) >= max_load_stop_threshold)
        {
           ...
           // controller_config.joint_enabled=false;
            return error;
...
//    }
   ...
   impedance_terms.torque_feedback=dterm_mNm;
   ...
   impedance_terms.torque_tot= torque_setpoint_mNm;  //valore originale non limitato dalla soglia max_torque_control_input
...
void sensojoint_control::set_inverse_dynamics_parameters(){
    torque_terms.weight_assistance = 1.2; // (1.2)
    torque_terms.compensation_factor = 0.99; //(0.99)
...
compute_inverse_dynamics(double position_rad){
    ...
    double weight_assistance     = torque_terms.weight_assistance; // human user weight correction
    double gravity_weight_factor = torque_terms.compensation_factor; //robot weight correction
  ...
  //return (-1)*gravity_weight_factor*gravity_torque_robot + Inertia_torque_mNm;// - weight_assistance*weight_compensation ;
  return (-1)*weight_assistance*weight_compensation - gravity_weight_factor*gravity_torque_robot + Inertia_torque_mNm;
