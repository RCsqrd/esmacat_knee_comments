...
enum robot_control_mode{
    ...
    adaptive_traj_control   = 210,
    target_position_control = 211,
    go_to_position          = 212,
};
...
static map< robot_control_mode, const char * > robot_control_labels = {
    ...
    {adaptive_traj_control,     "ADP-CONTROL" },
    {target_position_control,     "TARGET_POS_CONTROL" },
...
struct motor_values{
    ...
    double position_actual_value_rad=0;
...
struct  torque_control_terms
{
    ...
    float max_load_stop_threshold = 30000;          // [mNm]max value allowed as load cell reading  (15000)
...
struct impedance_control_terms{
    ...
    double T_stops_max_torque;    //[mNm] max allowed torque of the soft stop contribution (10000.0)
