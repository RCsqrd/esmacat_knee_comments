...
void sensojoint_manager::init()
{
    ...
    sensojoint_manager::initialize_write2file_adaptive_traj_data();
//    sensojoint_manager::read_file();
    ...
    ecat_sensojoint.set_impedance_control_Kgain(50*Nm_to_mNm);
    ecat_sensojoint.set_impedance_control_Dgain(5*Nm_to_mNm);

    //MODIFY : use this function to update the soft stop from shared memory
    ecat_sensojoint.set_soft_stop_gain(30,2.5);   //(stiffness [Nm/rad], damping [Nm/(rad/sec)] )
    ecat_sensojoint.set_soft_stop_parameters(10.0, -74.0, 5, true); // [deg] lim up - lim low - amplitude soft stop - reset position
    total_rep_counter=5;
...
//                   Pm2_actual=Pm2_actual+(error_traj^(2))/(total_rep_counter*(EXERCISE_DURATION-time_scaling_f*Pm2_average)));
             ...
             //      PLOGW<<" Gravity Torque"<<weight_compensation_torque<<endl;}
                     ...
                     if (static_cast<int>(elapsed_time_ms_offset_exercise)%50 == 0) {
                         sensojoint_manager::write2file_adaptive_traj_data();
                     }
                        // Update Status
                count++;
...
// it can be changed to a function of sensojoint control and then an update of a structure in struct that cointains all the indexes
     ...
     //                }
                ...
                if (count == static_cast<int>(EXERCISE_DURATION-time_scaling_f*Error_average) ){
                    count = 0;
                    repetition_counter=repetition_counter+1;
                    if (ind_good==0) {ind_good=1;}
                    if (ind_good==1) {ind_good=0;}
     //               int i=0;
...
void sensojoint_manager::write2file() {
    ...
    CSVfile << endl
            ...
            << impedance_terms_interim.torque_feedback;    //FIX salva i dati solo nella modality traiettoria

}

void sensojoint_manager::initialize_write2file_adaptive_traj_data()
{
...
void sensojoint_manager::write2file_adaptive_traj_data() {
