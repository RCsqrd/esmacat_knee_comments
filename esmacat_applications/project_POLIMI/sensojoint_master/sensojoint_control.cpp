...
void sensojoint_control::sensojoint_control_treshold_parameters() {
    ...
    torque_terms.max_load_stop_threshold = 32500;   // [mNm]max value allowed as load cell reading

    impedance_terms.T_stops_max_torque = 20000;     //[mNm]
    impedance_terms.impedance_max_error_rad = 0.5;
    impedance_terms.max_torque_control_input = 32000;   // [mNm]
...
//    }
    ...
    if (derating_param_time_to_arrive_75p_in_msec < 1e-5){
        ...
        derating_factor = smooth_start_func_tanh(elapsed_time_ms/derating_param_time_to_arrive_75p_in_msec);
...
{
    esmacat_err error = NO_ERR;
    double ref_position = esmacat_sensojoint::get_position_actual_value_rad()- (M_PI*340)/180;
...
esmacat_err sensojoint_control::impedance_control(float setpoint_radians, float inverse_dynamics_torque_mNm, float elapsed_time_ms)
{
    ...
    double measured_position_radians = (esmacat_sensojoint::get_position_actual_value_rad() - (M_PI*340)/180);
...
//    }
   ...
   torque_terms.control_output=static_cast<double>(torque_setpoint_mNm);
...
void sensojoint_control::set_inverse_dynamics_parameters(){
    torque_terms.weight_assistance = 1.2;
