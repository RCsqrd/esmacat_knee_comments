...
int main() {
    ...
    //Wait for the change of control mode before proceeding
    ...
    while(Traj_sm.data->controller_config.control_mode == robot_control_mode::transparent_control) {
     ...
     //       cout<< Traj_sm.data->motor_terms_sm.position_actual_value;
        if( (Traj_sm.data->adaptive_traj_ref_sm.traj_is_adapted == false ) && (Traj.repetition_counter<=Traj.rep_number) )  {
         // Set nominal beta function parameters
         Traj.set_nominal_beta_function_param(EXERCISE_START,EXERCISE_AMPLITUDE,EXERCISE_DURATION,5);
         // Compute trajectory reference
        ...
        Traj.read_ad_file();// Read data from a file
        Traj.discard_rep(); // Discard invalid repetitions
        
        // If there are no valid repetitions left, exit the program
        if (Traj.good_rep_list.size() ==0) {
            return 0;
        }
        Traj.compute_average_quantities();// Compute average quantities from the data
        Traj.write_ad_beta_param(); // Write parameters to a file
        Traj_sm.data->adaptive_traj_ref_sm = Traj.get_adapt_offline_param(); // Update shared memory with adaptive parameters
        Traj_sm.data->adaptive_traj_ref_sm.traj_is_adapted = true; // Mark the trajectory as adapted
