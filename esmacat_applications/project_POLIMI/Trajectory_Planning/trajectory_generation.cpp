...
/*
The function trajectory computation 
extracts from each transparent execution and saves the following variables:
...
void trajectory_gen::compute_average_quantities() {
  ...
  for (int k=0; k<good_rep_list.size(); k++)  {
    ...
    if (jp+j < (data_reps_array.size()-1)) {
      ...
      pos_err_time_average_t=data_reps_array[jp+j+1][2];      
      vel_err_time_average_t=data_reps_array[jp+j+1][4];
        if  (k==0)  {
