...
void my_app::assign_slave_sequence(){
    ...
    assign_esmacat_slave_index(&ease_1,0);
    assign_esmacat_slave_index(&ease_2,1);

    // Log the position of the slave onto the terminal
    std :: cout << "EASE Slaves at index 0 and 1" << std :: endl;
}
...
void my_app::loop(){
    ...
    ros_ease_1.set_read_registers(&ease_1);
    ros_ease_2.set_read_registers(&ease_2);

    // The newly modified qrite registers from the ROS Communication is returned from the shared memory 
    ros_message_1 = ros_ease_1.get_write_registers();
    
    ros_message_2 = ros_ease_2.get_write_registers();
    ...
    ease_1.set_output_variable_5_OUT_GEN_INT5(ros_message_1.INT5);
    ease_2.set_output_variable_5_OUT_GEN_INT5(ros_message_2.INT5);
}
