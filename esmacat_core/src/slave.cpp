...
{
    PLOGI << "esmacat_slave object has been created!" ;
}

/** @brief Destructor for the EtherCAT slave class */
esmacat_slave::~esmacat_slave()
{
    PLOGI << "esmacat_slave object has been destructed" ;
}
...
int esmacat_slave::flush_one_set_of_system_parameters()
{
    ...
    else
    {
        ...
        {
        std::cout << "succeed for setup, setup type: " << std::hex << OUT_system_parameter_type
                  << "  setup value: " << std::dec << OUT_system_parameter_value << std::endl;
